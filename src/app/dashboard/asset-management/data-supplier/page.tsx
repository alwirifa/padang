...
const UsersPage: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `https://giraffe-adjusted-severely.ngrok-free.app/api/admin/supplier?id=${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "ngrok-skip-browser-warning": "69420",
  ...
  const handleUpdate = async (values: z.infer<typeof formSchema>) => {
    ...
    toast.promise(
      axios
        .put(
          `https://giraffe-adjusted-severely.ngrok-free.app/api/admin/supplier?id=${selectedSupplier.id}`,
          values,
          {
            headers: {
              Authorization: `Bearer ${token}`,
