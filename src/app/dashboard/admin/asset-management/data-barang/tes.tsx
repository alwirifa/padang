...
const UsersPage: React.FC = () => {
  ...
  const fetchBarang = async () => {
    try {
      ...
      const response = await axios.get(
        "https://giraffe-adjusted-severely.ngrok-free.app/api/admin/user",
        {
          headers: {
  ...
  const handleDelete = async (id: number) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `https://giraffe-adjusted-severely.ngrok-free.app/api/admin/user?id=${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "ngrok-skip-browser-warning": "69420",
  ...
  const handleUpdate = async (
    ...
    toast.promise(
      axios
        .put(
          `https://giraffe-adjusted-severely.ngrok-free.app/api/admin/user?id=${selectedUser?.id}`,
          values,
          {
            headers: {
              Authorization: `Bearer ${token}`,
